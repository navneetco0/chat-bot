...
import { getParticularChat, setChat } from '../Chat/action'
...
export const loginUser = (form) => (dispatch) => {
  ...
  axios
    ...
    .then((res) => {dispatch(userData(res.data)); dispatch(setChat(res.data.chats))})
    .catch((error) => dispatch(loginError(error)))
}
...
export const getData = (token) => (dispatch) => {
  ...
  axios
    ...
    .then((res) => {
      dispatch(userData(res.data))
      dispatch(getParticularChat({ id: res.data.id, token: token }))
    })
    .catch((error) => dispatch(loginError(error)))
}
